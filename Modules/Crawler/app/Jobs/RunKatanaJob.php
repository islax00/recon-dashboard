...
use Modules\Crawler\Services\KatanaService;
use Modules\Reconnaissance\Models\Scan;

class RunKatanaJob implements ShouldQueue
{
    ...
    public function handle(KatanaService $service): void
    {
        ...
        $this->logScan(
            $this->scan,
            $service->name(),
            sprintf('Discovered %d endpoints', count($result->items)),
        );
    }
}
